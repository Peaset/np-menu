...
TriggerEvent('es:addCommand', 'meslek', function(source)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local job = xPlayer.job.label
    local jobgrade = xPlayer.job.grade_label

--TriggerClientEvent('esx:showNotification', _source, 'Mesleğin: ~g~' .. job .. ' ~s~-~g~ ' .. jobgrade)
TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Mesleğin: ' .. job .. ' - ' .. jobgrade})  
end, {help = "Check what job you have"})

TriggerEvent('es:addCommand', 'para', function(source)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
...
TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Üzerindeki Paran $' .. wallet .. ''})
...
TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = '$' .. bank .. ' Bankanda Var'})
...
TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = '$' .. black_money .. ' Üzerinde Kara Paran Var'})
    ...
    if xPlayer.job.grade_name == 'boss' then
		...
		if society ~= nil then
			...
			money = 0
                ...
                --TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Mesleğin: ' .. job .. ' - ' .. jobgrade})
                Citizen.Wait(1500)
                TriggerClientEvent('esx:showNotification', _source, ' ~g~$~g~' .. wallet .. ' ~s~in Üzerindeki Paran~g~ ')
                --TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'You currently have $' .. wallet .. ' in your wallet'})
                Citizen.Wait(1500)
                TriggerClientEvent('esx:showNotification', _source, '~g~$~g~' .. bank .. ' ~s~Bankanda Var~g~ ')
                --TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'You currently have $' .. bank .. ' in your bank'})
                Citizen.Wait(1500)
                TriggerClientEvent('esx:showNotification', _source, '~g~$~g~' .. black_money .. ' ~s~Üzerinde Kara Paran Var~g~ ')
