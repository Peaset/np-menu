...
RegisterCommand("yenile", function()
	if not IsEntityPlayingAnim(GetPlayerPed(-1), "mp_arresting", "idle", 3) and isDead then
...
function SendDistressSignalPolice()
	...
	TriggerServerEvent('esx_phone:send', 'police', 'Yaralı polis memuru/sheriff, acil destek gerekiyor' , false, {
		x = coords.x,
		y = coords.y,
		z = coords.z
	})
	TriggerServerEvent('esx_phone:send', 'sheriff', 'Yaralı polis memuru/sheriff, acil destek gerekiyor' , false, {
		x = coords.x,
		y = coords.y,
		z = coords.z
	})
...
AddEventHandler('esx_ambulancejob:useItem', function(itemName)
	...
	if itemName == 'medikit' then
		...
		ESX.Streaming.RequestAnimDict(lib, function()
			...
			TriggerEvent('notification', 'You used a medkit used')
		end)
	...
	elseif itemName == 'bandage' then
		...
		ESX.Streaming.RequestAnimDict(lib, function()
			...
			TriggerEvent('notification', 'You used a bandage used')
		end)
	end
end) 
...
function StartDistressSignal()
	Citizen.CreateThread(function()
		...
		while timer > 0 and IsDead do
			...
			EndTextCommandDisplayText(0.456, 0.90)
