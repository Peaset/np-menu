	...
	TriggerEvent('skinchanger:getSkin', function(skin)
		if skin.sex == 0 then
			if Config.Uniforms[job].male then
				...
				exports['mythic_notify']:DoHudText('inform', (_U('no_outfit')))
			end
			
			if job == 'bullet_wear' then
				SetPlayerMaxArmour(PlayerId(),200)
				SetPedArmour(PlayerPedId(), 100)
			elseif job ~= "citizen_wear" then
...
function OpenCloakroomMenu()
	...
	elseif grade == 'recruit1' then
		table.insert(elements, {label = _U('police_wear'), value = 'recruit1_wear'})
	...
	elseif grade == 'officer1' then
		table.insert(elements, {label = _U('police_wear'), value = 'officer1_wear'})
	...
	elseif grade == 'intendent1' then
		table.insert(elements, {label = _U('police_wear'), value = 'intendent1_wear'})
	...
	if Config.EnableNonFreemodePeds then
		table.insert(elements, {label = 'Polis Wear', value = 'freemode_ped', maleModel = 's_m_y_cop_01', femaleModel = 's_f_y_cop_01'})
		table.insert(elements, {label = 'S.W.A.T Kıyafeti', value = 'freemode_ped', maleModel = 's_m_y_swat_01', femaleModel = 's_m_y_swat_01'})
	end
			...
			if Config.MaxInService ~= -1 then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						exports['mythic_notify']:DoHudText('inform', (_U('service_out')))
					end
				end, 'police')
			end
		end
				...
				if not isInService then
					...
					ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount)
						if not canTakeService then
							exports['mythic_notify']:DoHudText('inform', (_U('service_max', inServiceCount, maxInService)))
						else
							serviceOk = true
							...
							TriggerServerEvent('esx_service:notifyAllInService', notification, 'police')
							TriggerEvent('esx_policejob:updateBlip')
							exports['mythic_notify']:DoHudText('inform', (_U('service_in')))
						end
					end, 'police')
		...
		if data.current.action == 'buy_vehicle' then
			...
			elseif type == 'helicopter' then
				...
				if #authorizedHelicopters > 0 then
					...
					exports['mythic_notify']:DoHudText('inform', (_U('helicopter_notauthorized')))
					return
				end
			end
						...
						if data2.current.stored then
							...
							if foundSpawn then
								...
								ESX.Game.SpawnVehicle(data2.current.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
									...
									TriggerServerEvent("tuninglaptop:checkAndSync",data2.current.vehicleProps.plate,NetworkGetNetworkIdFromEntity(vehicle))
									TriggerServerEvent('esx_vehicleshop:setJobVehicleState', data2.current.vehicleProps.plate, false)
									exports['mythic_notify']:DoHudText('inform', (_U('garage_released')))
									local plate = GetVehicleNumberPlateText(vehicle)
                                    exports['hsn-hotwire']:AddKeys(plate)
								end)
							end
						else
							exports['mythic_notify']:DoHudText('inform', (_U('garage_notavailable')))
						end
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('esx_policejob:storeNearbyVehicle', function(storeSuccess, foundNum)
		if storeSuccess then
			...
			IsBusy = false
			exports['mythic_notify']:DoHudText('inform', (_U('garage_has_stored')))
		else
			exports['mythic_notify']:DoHudText('inform', (_U('garage_has_notstored')))
		end
	end, vehiclePlates)
end
...
function GetAvailableVehicleSpawnPoint(station, part, partNum)
	...
	if found then
		...
		exports['mythic_notify']:DoHudText('inform', (_U('vehicle_blocked')))
		return false
	end
end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_policejob:buyJobVehicle', function (bought)
					if bought then
						exports['mythic_notify']:DoHudText('inform', (_U('vehicleshop_bought', data.current.name, ESX.Math.GroupDigits(data.current.price))))
...
function TelsizKoduSor()
	...
	function(data2, menu2)
		TriggerServerEvent('esx_policejob:telsizKodu',string.gsub(string.upper(data2.value)," ","-"))
		menu2.close()
	end, function(data2, menu2)
		menu2.close()
	end)
...
local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
			if closestPlayer == -1 or closestDistance > 3.0 then
				exports['mythic_notify']:DoHudText('error', 'Yakında Fatura Kesilecek Kimse Yok')
				return
end
ESX.UI.Menu.Open(
		'dialog', GetCurrentResourceName(), 'jail_choose_time_menu',
	...
	local amount = tonumber(data2.value)
	    if amount == nil then
			exports['mythic_notify']:DoHudText('inform', (_U('invalid_amount')))
		else
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		elements = {
			...
			{label = 'Özel Fatura Kes', value = 'create_bill'},
			{label = "Hapishane Menusu", value = 'jail_menu'},
			{label = "Gps Kodu Belirle", value = 'telsiz_kodu'}
	}}, function(data, menu)
	    if data.current.value == 'jail_menu' then
            TriggerEvent("possiblejail:openJailMenu")
        end  
	    if data.current.value == 'telsiz_kodu' then
            TelsizKoduSor()
        end
		...
		if data.current.value == 'citizen_interaction' then
			...
			local elements = {
				...
				{label = _U('handcuffq'), value = 'handcuffhard'},
				{label = 'Kelepçe kır', value = 'handcuffbreak'},
            ...
            if grade == 'lieutenant' then
                table.insert(elements, {label = _U('revive player'), value = 'revive'}) 
            elseif grade == 'sergeant' then                
                table.insert(elements, {label = _U('revive player'), value = 'revive'})				
            elseif grade == 'boss' then                
                table.insert(elements, {label = _U('revive player'), value = 'revive'})                		
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					if health == 0 then
						...
						ClearPedTasks(playerPed)
							...
							exports['mythic_notify']:DoHudText('black', (_U('revive_complete')))
							else
							exports['mythic_notify']:DoHudText('inform', (_U('isdead')))
							end
						end)
					end
					...
					local target_id = GetPlayerServerId(target)					    
						TriggerServerEvent('kelepcele:requestarrest', target_id, playerheading, playerCoords, playerlocation)
						exports['mythic_notify']:DoHudText('inform', "Kelepçeledin")
					elseif action == 'handcuffbreak' then
						openBreakCuff()
					...
					exports['mythic_notify']:DoHudText('inform', (_U('no_players_nearby')))
				end
			end, function(data2, menu2)
				menu2.close()
			end)			
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_interaction', {
				...
				elseif DoesEntityExist(vehicle) then
					...
					elseif action == 'impound' then
                        ...
                        exports['progressBars']:startUI(10000, "Araç bağlanıyor")
						...
						--[[Citizen.CreateThread(function()
							while currentTask.busy do
								...
								if not DoesEntityExist(vehicle) and currentTask.busy then
									exports['mythic_notify']:DoHudText('inform', (_U('impound_canceled_moved')))
					...
					exports['mythic_notify']:DoHudText('inform', (_U('no_vehicles_nearby')))
				end
...
function openCardMenu()	
    specialMenu("Yakındaki Kelepçelenebilecek Kişiler",function(nearbyPlayer)  
		OpenIdentityCardMenu(nearbyPlayer)
	end)	
end
...
function openBodySearchMenu()
	specialMenu("Üst Arama Menüsü",function(nearbyPlayer)
		TriggerServerEvent('esx_policejob:message', nearbyPlayer, _U('being_searched'))
		OpenBodySearchMenu(nearbyPlayer)
	end)
end
...
AddEventHandler("police:cuffMenu",function()
	openCuffMenu()
end)

AddEventHandler("police:hardcuffMenu",function()
	openHardCuffMenu()
end)

AddEventHandler("police:breakcuffMenu",function()
	openBreakCuff()
end)
...
function openCuffMenu()	
	specialMenu("Yakındaki Kelepçelenebilecek Kişiler",function(nearbyPlayer) 
		TriggerServerEvent('kelepcele:softKelepce', nearbyPlayer) 
	end)
end
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("police")) then --Insert VehicleModelName here.
			SetVehicleEnginePowerMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), true), 16.0)
			SetVehicleEngineTorqueMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), true), 128.0)
		end
	end
end)
function openHardCuffMenu()
	specialMenu("Yakındaki Kelepçelenebilecek Kişiler",function(nearbyPlayer)
		playerheading = GetEntityHeading(PlayerPedId())
		playerlocation = GetEntityForwardVector(PlayerPedId())
		playerCoords = GetEntityCoords(PlayerPedId())
		TriggerServerEvent("kelepcele:kelepce",nearbyPlayer,playerheading,playerlocation,playerCoords)
	end)
end

function openBreakCuff()
	specialMenu("Kelepçe Kırma - Yakındaki Kişiler",function(nearbyPlayer)
		 playerheading = GetEntityHeading(PlayerPedId())
		 playerlocation = GetEntityForwardVector(PlayerPedId())
		 playerCoords = GetEntityCoords(PlayerPedId())
		local target_id = nearbyPlayer					    
		exports['mythic_notify']:DoHudText('inform', "Kelepçe kırmaya başladın.")	
		TriggerServerEvent('kelepce:zorlaAc', target_id, playerheading, playerCoords, playerlocation)
	end)
end


function openHardunCuffMenu()
	specialMenu("Yakındaki Kelepçesi Çözülecek Kişiler",function(nearbyPlayer)					
  			playerheading = GetEntityHeading(PlayerPedId())
   			playerlocation = GetEntityForwardVector(PlayerPedId())
			playerCoords = GetEntityCoords(PlayerPedId())
		local target_id = nearbyPlayer					    
...
function openDragMenu()
	specialMenu("Yakındaki Taşınacak Kişiler",function(nearbyPlayer)
		TriggerServerEvent('esx_policejob:drag', nearbyPlayer)
	end)	
end

function openPutinMenu()
	specialMenu("Yakındaki Araca Bindirilcek Kişiler",function(nearbyPlayer)
		TriggerServerEvent('esx_policejob:putInVehicle', nearbyPlayer)	
	end)	
end

function openPutoutMenu()
	specialMenu("Yakındaki Araca Indirilcek Kişiler",function(nearbyPlayer)
		TriggerServerEvent('esx_policejob:OutVehicle', nearbyPlayer)
	end)	
end


function openFinesMenu()
	specialMenu("Yakındaki Ceza Yazılacak Kişiler",function(nearbyPlayer)
		OpenFineMenu(nearbyPlayer)
	end)	
end

function openLicenseMenu()
	specialMenu("Yakındaki Lisansa Bakılcak Kişiler",function(nearbyPlayer)
		ShowPlayerLicense(nearbyPlayer)
	end)	
end

function openCommunityServiceMenu()
	specialMenu("Yakındaki Kamu Hizmeti Alıcak Kişiler",function(nearbyPlayer)	
		SendToCommunityService(nearbyPlayer)
	end)			
end

function openUnFineMenu()
	specialMenu("Yakındaki Kamu Hizmeti Alıcak Kişiler",function(nearbyPlayer)
...
function openGSR()
	specialMenu("Yakındaki GSR Testi Yapılacak Kişiler",function(nearbyPlayer)
		TriggerServerEvent('GSR:Status2', nearbyPlayer)			
	end)
end
...
function OpenBodySearchMenu(player)
    ...
    TriggerServerEvent('esx_policejob:message', GetPlayerServerId(player), _U('being_searched'))
end
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			exports['mythic_notify']:DoHudText('inform', (_U('search_database_error_invalid')))
		else
			ESX.TriggerServerCallback('esx_policejob:getVehicleFromPlate', function(owner, found)
				if found then
					exports['mythic_notify']:DoHudText('inform', (_U('search_database_found', owner)))
				else
					exports['mythic_notify']:DoHudText('inform', (_U('search_database_error_not_found')))
				end
			end, data.value)
			menu.close()
		end
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			exports['mythic_notify']:DoHudText('inform', (_U('licence_you_revoked', data.current.label, targetName)))
			...
			ESX.TriggerServerCallback('esx_policejob:buyWeapon', function(bought)
				if bought then
					if data.current.price > 0 then
						exports['mythic_notify']:DoHudText('inform', (_U('armory_bought', data.current.weaponLabel, ESX.Math.GroupDigits(data.current.price))))
					end

					menu.close()
					OpenBuyWeaponsMenu()
				else
					exports['mythic_notify']:DoHudText('inform', (_U('armory_money')))
				end
...
function OpenWeaponComponentShop(components, weaponName, parentShop)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons_components', {
		...
		if data.current.hasComponent then
			exports['mythic_notify']:DoHudText('inform', (_U('armory_hascomponent')))
		else
			ESX.TriggerServerCallback('esx_policejob:buyWeapon', function(bought)
				if bought then
					if data.current.price > 0 then
						exports['mythic_notify']:DoHudText('inform', (_U('armory_bought', data.current.componentLabel, ESX.Math.GroupDigits(data.current.price))))
					end
	...
	ESX.TriggerServerCallback('esx_policejob:getStockItems', function(items)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					exports['mythic_notify']:DoHudText('inform', (_U('quantity_invalid')))
				else
					menu2.close()
					menu.close()
...
AddEventHandler('esx_policejob:handcuff', function(force)
	if force == nil then
	...
	Citizen.CreateThread(function()
		if IsHandcuffed then
			...
			exports['mythic_notify']:DoHudText('inform', "Kelepçelendin")
            ...
            exports['mythic_notify']:DoHudText('inform', "Kelepçen çözüldü")			
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job and (PlayerData.job.name == 'police' or PlayerData.job.name == 'ambulance') then
			ESX.TriggerServerCallback("esx_policejob:GPSKontrol",function(telsiz)
				if not telsiz then
					TriggerServerEvent("esx_policejob:telsizKodu",nil)
				end
			end)
		end
	end
...
Citizen.CreateThread(function()
	while true do
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'police' then
				...
				elseif CurrentAction == 'menu_armory' then
					...
					elseif playerInService then
						...
						exports['mythic_notify']:DoHudText('inform', (_U('service_not')))
					end
		...
		if IsControlJustReleased(0, 38) and currentTask.busy then
			exports['mythic_notify']:DoHudText('inform', (_U('impound_canceled')))
			ESX.ClearTimeout(currentTask.task)
...
function createBlip(id,tk,notEMS)
...
AddEventHandler('esx_policejob:updateBlip', function(tKs)
	if tKs ~= nil then
		telsizKodlari = tKs
	end
	-- Refresh all blips
	...
	if (PlayerData.job and PlayerData.job.name == 'police') or (PlayerData.job and PlayerData.job.name == 'ambulance') then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				local isPolice = (players[i].job.name == 'police')
				local isEMS = (players[i].job.name == "ambulance")
				if isPolice or isEMS then
					local id = GetPlayerFromServerId(players[i].source)
					local tk = telsizKodlari[players[i].source]
					if NetworkIsPlayerActive(id) and GetPlayerPed(id) ~= PlayerPedId() and tk ~= nil then 
						if isEMS then
							createBlip(id,tk,false)
						else
							createBlip(id,tk,true)
						end
					end
				end
			end
...
function StartHandcuffTimer()
	...
	handcuffTimer.task = ESX.SetTimeout(Config.handcuffTimer, function()
		exports['mythic_notify']:DoHudText('inform', (_U('unrestrained_timer')))
		TriggerEvent('esx_policejob:unrestrain')
...
function ImpoundVehicle(vehicle)
	...
	exports['mythic_notify']:DoHudText('inform', (_U('impound_successful')))
	currentTask.busy = false
end
