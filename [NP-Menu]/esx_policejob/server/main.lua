...
AddEventHandler("esx:userCreated",function(src)
	Citizen.Wait(5000)
	local xPlayer = ESX.GetPlayerFromId(src)
	xPlayer.addMoney(1000) -- Başlangıç parası
end)
...
ESX.RegisterServerCallback("esx_policejob:getMeNames",function(source,callback,ids)
	local identities = {}
...
AddEventHandler('esx_policejob:forceBlip', function()
	TriggerClientEvent('esx_policejob:updateBlip', -1, telsizKodlari)
end)

RegisterServerEvent('esx_policejob:telsizKodu')
AddEventHandler('esx_policejob:telsizKodu', function(telsizKodu)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getInventoryItem("gpsill").count > 0 then
		telsizKodlari[source] = telsizKodu
		TriggerClientEvent('possiblenotify:client:SendAlert', xPlayer.source, { type = 'inform', text ="GPS'in devrede, en gec 60 saniye icerisinde haritada belireceksin." })
	elseif telsizKodu ~= nil then
		TriggerClientEvent('possiblenotify:client:SendAlert', xPlayer.source, { type = 'error', text ="GPS'in yok!" })
	elseif telsizKodu == nil then
		telsizKodlari[source] = nil
	end
end)

ESX.RegisterServerCallback("esx_policejob:GPSKontrol",function(source,cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getInventoryItem("gpsill").count > 0 then
		cb(true)
	else
